...
func TestTaskExec_Validate(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			// WHEN
			err := execTasks.Validate(context.Background())
...
func TestTaskExec_Ask(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"should bubble error if fail to select task in default cluster": {
			...
			setupMocks: func(m execTaskMocks) {
				...
				m.taskSel.EXPECT().RunningTask(gomock.Any(), taskExecTaskPrompt, taskExecTaskHelpPrompt,
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			mockNewTaskSel := func(_ aws.Config) runningTaskSelector {
				return mockTaskSel
			}
...
func TestTaskExec_Execute(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"should bubble error if fail to execute commands": {
			...
			setupMocks: func(m execTaskMocks) {
				m.provider.EXPECT().DefaultConfig(gomock.Any()).Return(aws.Config{}, nil)
				m.commandExec.EXPECT().ExecuteCommand(gomock.Any(), ecs.ExecuteCommandInput{
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			mockNewCommandExec := func(_ aws.Config) ecsCommandExecutor {
