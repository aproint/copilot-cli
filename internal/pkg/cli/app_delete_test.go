	...
	tests := map[string]struct {
		...
		"success deleting all the resources along with workspace summary": {
			...
			setupMocks: func(mocks deleteAppMocks) {
				gomock.InOrder(
					// delete pipelines
					mocks.codepipeline.EXPECT().ListDeployedPipelines(context.Background(), mockAppName).Return(mockPipelines, nil),
					...
					// deleteTasks
					mocks.deployer.EXPECT().ListTaskStacks(context.Background(), mockAppName, mockEnvs[0].Name).Return(mockTaskStacks, nil),
					...
					mocks.deployer.EXPECT().GetRegionalAppResources(context.Background(), mockApp).Return(mockResources, nil),
					mocks.spinner.EXPECT().Start(deleteAppCleanResourcesStartMsg),
					mocks.bucketEmptier.EXPECT().EmptyBucket(context.Background(), mockResources[0].S3Bucket).Return(nil),
					mocks.spinner.EXPECT().Stop(log.Ssuccess(deleteAppCleanResourcesStopMsg)),

					// deleteAppResources
					mocks.deployer.EXPECT().DeleteApp(context.Background(), mockAppName).Return(nil),
