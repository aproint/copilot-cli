	...
	testCases := map[string]struct {
		...
		"prompts for app name if empty": {
			...
			callMocks: func(m deletePipelineMocks) {
				...
				m.deployedPipelineLister.EXPECT().ListDeployedPipelines(ctx, testAppName).Return([]deploy.Pipeline{
					{
		...
		"errors if passed-in pipeline name is invalid": {
			...
			callMocks: func(m deletePipelineMocks) {
				...
				m.deployedPipelineLister.EXPECT().ListDeployedPipelines(ctx, testAppName).Return([]deploy.Pipeline{}, nil)
			},
		...
		"gets name of legacy pipeline": {
			...
			callMocks: func(m deletePipelineMocks) {
				m.store.EXPECT().GetApplication(ctx, testAppName).Return(nil, nil)
				m.sel.EXPECT().DeployedPipeline(ctx, gomock.Any(), gomock.Any(), testAppName).Return(deploy.Pipeline{
					Name:     testPipelineName,
					IsLegacy: true,
				}, nil)
			},
		...
		"error getting pipeline": {
			...
			callMocks: func(m deletePipelineMocks) {
				m.store.EXPECT().GetApplication(ctx, testAppName).Return(nil, nil)
				m.sel.EXPECT().DeployedPipeline(ctx, gomock.Any(), gomock.Any(), testAppName).Return(deploy.Pipeline{}, errors.New("some error"))
			},
...
func TestDeletePipelineOpts_Execute(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"skips delete secret confirmation (and deletion attempt) if there is no secret": {
			...
			setupMocks: func(mocks deletePipelineMocks) {
				gomock.InOrder(
					mocks.secretsmanager.EXPECT().DescribeSecret(ctx, testPipelineSecret).Return(nil, &secretsmanager.ErrSecretNotFound{}),
					mocks.secretsmanager.EXPECT().DeleteSecret(ctx, gomock.Any()).Times(0),
					mocks.prog.EXPECT().Start(fmt.Sprintf(fmtDeletePipelineStart, testPipelineName, testAppName)),
					mocks.deployer.EXPECT().DeletePipeline(ctx, targetPipeline).Return(nil),
		...
		"skips secret deletion if secret found but tags don't match": {
			...
			setupMocks: func(mocks deletePipelineMocks) {
				gomock.InOrder(
					mocks.secretsmanager.EXPECT().DescribeSecret(ctx, testPipelineSecret).Return(mockBadResp, nil),
					mocks.prog.EXPECT().Start(fmt.Sprintf(fmtDeletePipelineStart, testPipelineName, testAppName)),
					mocks.deployer.EXPECT().DeletePipeline(ctx, targetPipeline).Return(nil),
					mocks.prog.EXPECT().Stop(log.Ssuccessf(fmtDeletePipelineComplete, testPipelineName, testAppName)),
				)
		...
		"wraps error from DescribeSecret": {
			...
			setupMocks: func(mocks deletePipelineMocks) {
				gomock.InOrder(
					mocks.secretsmanager.EXPECT().DescribeSecret(ctx, testPipelineSecret).Return(nil, errors.New("some error")),
				)
			},
		...
		"skips delete secret confirmation when flag is specified": {
			...
			setupMocks: func(mocks deletePipelineMocks) {
				gomock.InOrder(
					mocks.secretsmanager.EXPECT().DescribeSecret(ctx, testPipelineSecret).Return(mockResp, nil),
					// no confirmation prompt for deleting secret
					mocks.secretsmanager.EXPECT().DeleteSecret(ctx, testPipelineSecret).Return(nil),
					mocks.prog.EXPECT().Start(fmt.Sprintf(fmtDeletePipelineStart, testPipelineName, testAppName)),
					mocks.deployer.EXPECT().DeletePipeline(ctx, targetPipeline).Return(nil),
					mocks.prog.EXPECT().Stop(log.Ssuccessf(fmtDeletePipelineComplete, testPipelineName, testAppName)),
				)
		...
		"does not delete secret if user does not confirm": {
			...
			setupMocks: func(mocks deletePipelineMocks) {
				gomock.InOrder(
					...
					mocks.secretsmanager.EXPECT().DeleteSecret(ctx, testPipelineSecret).Times(0),
					mocks.prog.EXPECT().Start(fmt.Sprintf(fmtDeletePipelineStart, testPipelineName, testAppName)),
					mocks.deployer.EXPECT().DeletePipeline(ctx, targetPipeline).Times(1).Return(nil),
					mocks.prog.EXPECT().Stop(log.Ssuccessf(fmtDeletePipelineComplete, testPipelineName, testAppName)),
				)
			},
			wantedError: nil,
		...
		"error when deleting stack": {
			...
			setupMocks: func(mocks deletePipelineMocks) {
				gomock.InOrder(
					...
					mocks.secretsmanager.EXPECT().DeleteSecret(ctx, testPipelineSecret).Return(nil),
					mocks.prog.EXPECT().Start(fmt.Sprintf(fmtDeletePipelineStart, testPipelineName, testAppName)),
					mocks.deployer.EXPECT().DeletePipeline(ctx, targetPipeline).Times(1).Return(testError),
