...
func TestDeployPipelineOpts_Ask(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			opts := deployPipelineOpts{ctx: context.Background(),
...
func TestDeployPipelineOpts_Execute(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"create and deploy pipeline": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					m.deployer.EXPECT().AddPipelineResourcesToApp(ctx, &app, region).Return(nil),
					m.prog.EXPECT().Stop(log.Ssuccessf(fmtPipelineDeployResourcesComplete, appName)).Times(1),

					// deployPipeline
					m.deployer.EXPECT().PipelineExists(ctx, gomock.Any()).Return(false, nil),
					m.deployer.EXPECT().GetAppResourcesByRegion(ctx, &app, region).Return(mockResource, nil),
					m.prog.EXPECT().Start(fmt.Sprintf(fmtPipelineDeployStart, pipelineName)).Times(1),
					m.deployer.EXPECT().CreatePipeline(ctx, gomock.Any(), gomock.Any()).Return(nil),
		...
		"update and deploy pipeline with new naming": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					// deployPipeline
					m.deployer.EXPECT().PipelineExists(ctx, gomock.Any()).Return(true, nil),
					m.deployer.EXPECT().GetAppResourcesByRegion(ctx, &app, region).Return(mockResource, nil),
					...
					m.deployer.EXPECT().UpdatePipeline(ctx, gomock.Any(), gomock.Any()).Return(nil),
		...
		"update and deploy pipeline with legacy naming": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					m.deployedPipelineLister.EXPECT().ListDeployedPipelines(ctx, appName).Return([]deploy.Pipeline{
						{
		...
		"returns an error if fail to add pipeline resources to app": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					// bootstrap pipeline resources
					m.prog.EXPECT().Start(fmt.Sprintf(fmtPipelineDeployResourcesStart, appName)).Times(1),
					m.deployer.EXPECT().AddPipelineResourcesToApp(ctx, &app, region).Return(errors.New("some error")),
		...
		"returns an error if unable to convert environments to deployment stage": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					m.ws.EXPECT().ReadPipelineManifest(pipelineManifestPath).Return(mockPipelineManifest, nil),
					m.ws.EXPECT().Rel(pipelineManifestPath).Return(relativePath, nil),
					m.actionCmd.EXPECT().Execute(gomock.Any()).Return(errors.New("some error")),
				)
			},
		...
		"returns an error if fails to get cross-regional resources": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					// getArtifactBuckets
					m.deployer.EXPECT().GetRegionalAppResources(ctx, gomock.Any()).Return(mockResources, errors.New("some error")),
				)
			},
		...
		"returns an error if fails to check if pipeline exists": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					// deployPipeline
					m.deployer.EXPECT().PipelineExists(ctx, gomock.Any()).Return(false, errors.New("some error")),
				)
			},
		...
		"returns an error if fails to create pipeline": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					m.deployer.EXPECT().GetAppResourcesByRegion(ctx, &app, region).Return(mockResource, nil),
					m.prog.EXPECT().Start(fmt.Sprintf(fmtPipelineDeployStart, pipelineName)).Times(1),
					m.deployer.EXPECT().CreatePipeline(ctx, gomock.Any(), gomock.Any()).Return(errors.New("some error")),
		...
		"returns an error if fails to update pipeline": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					m.prompt.EXPECT().Confirm(fmt.Sprintf(fmtPipelineDeployExistPrompt, pipelineName), "").Return(true, nil),
					m.prog.EXPECT().Start(fmt.Sprintf(fmtPipelineDeployProposalStart, pipelineName)).Times(1),
					m.deployer.EXPECT().UpdatePipeline(ctx, gomock.Any(), gomock.Any()).Return(errors.New("some error")),
		...
		"failed to fetch the template of deployed stack": {
			...
			callMocks: func(m deployPipelineMocks) {
				gomock.InOrder(
					...
					m.deployer.EXPECT().Template(ctx, gomock.Any()).Return("", fmt.Errorf("some error")))
			},
		...
		"failed prompt to accept diff": {
			...
			callMocks: func(m deployPipelineMocks) {
				...
				m.deployer.EXPECT().Template(ctx, gomock.Any()).Return("name: mockEnv\ntype: Environment", nil)
	...
	testCases := map[string]struct {
		...
		"getsBucketInfo": {
			mockDeployer: func(m *mocks.MockpipelineDeployer) {
				...
				m.EXPECT().GetRegionalAppResources(ctx, gomock.Any()).Return(mockResources, nil)
			},
			expectedOut: []deploy.ArtifactBucket{
				{
