...
func TestConfigRunner_CheckNonZeroExitCodeUsesCallerContext(t *testing.T) {
	...
	getter.EXPECT().HasNonZeroExitCode(gomock.Eq(ctx), []string{"task-1"}, "cluster").Return(nil)
	runner := ConfigRunner{Cluster: "cluster", NonZeroExitCodeGetter: getter}

	err := runner.CheckNonZeroExitCode(ctx, []*Task{{TaskARN: "task-1"}})
...
func TestNetworkConfigRunner_Run(t *testing.T) {
	testCases := map[string]struct {
		...
		"failed to get default cluster": {
			...
			mockClusterGetter: func(m *mocks.MockDefaultClusterGetter) {
				m.EXPECT().DefaultCluster(gomock.Any()).Return("", errors.New("error getting default cluster"))
			},
			MockVPCGetter: func(m *mocks.MockVPCGetter) {
			...
			mockStarter: func(m *mocks.MockRunner) {
				m.EXPECT().RunTask(gomock.Any(), gomock.Any()).Times(0)
			},
		...
		"failed to kick off tasks with input subnets and security groups": {
			...
			mockClusterGetter: func(m *mocks.MockDefaultClusterGetter) {
				m.EXPECT().DefaultCluster(gomock.Any()).Return("cluster-1", nil)
			},
			MockVPCGetter: func(m *mocks.MockVPCGetter) {
				m.EXPECT().SubnetIDs(gomock.Any(), []ec2.Filter{ec2.FilterForDefaultVPCSubnets}).Times(0)
			},
			mockStarter: func(m *mocks.MockRunner) {
				m.EXPECT().RunTask(gomock.Any(), gomock.Any()).Return(nil, errors.New("error running task"))
			},
		...
		"failed to get default subnets": {
			mockClusterGetter: func(m *mocks.MockDefaultClusterGetter) {
				m.EXPECT().DefaultCluster(gomock.Any()).AnyTimes()
			},
			MockVPCGetter: func(m *mocks.MockVPCGetter) {
				m.EXPECT().SubnetIDs(gomock.Any(), []ec2.Filter{ec2.FilterForDefaultVPCSubnets}).Return(nil, errors.New("error getting subnets"))
			},
			mockStarter: func(m *mocks.MockRunner) {
				m.EXPECT().RunTask(gomock.Any(), gomock.Any()).Times(0)
			},
		...
		"successfully kick off task with default subnets": {
			...
			MockVPCGetter: func(m *mocks.MockVPCGetter) {
				m.EXPECT().SubnetIDs(gomock.Any(), []ec2.Filter{ec2.FilterForDefaultVPCSubnets}).
		...
		"successfully kick off task with specified cluster": {
			...
			mockClusterGetter: func(m *mocks.MockDefaultClusterGetter) {
				m.EXPECT().DefaultCluster(gomock.Any()).Times(0)
			},
			MockVPCGetter: func(m *mocks.MockVPCGetter) {
				m.EXPECT().SubnetIDs(gomock.Any(), []ec2.Filter{ec2.FilterForDefaultVPCSubnets}).Times(0)
			},
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			tasks, err := task.Run(context.Background())
