...
func TestInitPipelineOpts_Ask(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"invalid app name": {
			...
			inAppName:   "ghost-app",
			setupMocks: func(m pipelineInitMocks) {
				m.store.EXPECT().GetApplication(ctx, "ghost-app").Return(nil, mockError)
			},
		...
		"invalid pipeline name": {
			...
			inGitBranch: "main",
			setupMocks: func(m pipelineInitMocks) {
				m.store.EXPECT().GetApplication(ctx, "my-app").Return(mockApp, nil)
			},
		...
		"success with GH repo with env and repoURL flags": {
			...
			setupMocks: func(m pipelineInitMocks) {
				...
				m.store.EXPECT().GetEnvironment(ctx, "my-app", "prod").Return(
					&config.Environment{
						Name: "prod",
					}, nil)
				m.pipelineLister.EXPECT().ListDeployedPipelinesWithContext(ctx, mockAppName).Return([]deploy.Pipeline{}, nil)
		...
		"no flags, prompts for all input, success case for selecting URL": {
			...
			setupMocks: func(m pipelineInitMocks) {
				...
				m.store.EXPECT().GetEnvironment(ctx, "my-app", "prod").Return(&config.Environment{
					Name:   "prod",
					Region: "us-west-2",
				}, nil)
		...
		"returns error if fail to get env config": {
			...
			setupMocks: func(m pipelineInitMocks) {
				...
				m.store.EXPECT().GetEnvironment(ctx, "my-app", "prod").Return(nil, errors.New("some error"))
			},
...
func TestInitPipelineOpts_Execute(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			// WHEN
			require.NoError(t, opts.parseRepoDetails(ctx))
